...
all:
	...
	$(info  build-docker         - build a docker image, IMAGE_NAME will default to $(PROJECT_NAME) if not specified!)
...
ifndef IMAGE_NAME
	...
	@docker build --build-arg PROJECT_NAME=$(PROJECT_NAME) --build-arg COMMIT=$(IMAGE_TAG) -t $(IMAGE_NAME):$(IMAGE_TAG) -f Dockerfile .
